...
static void cb030_cf_load_write_buffer(q9_board_t *b) {
    if (img_sec < Q9_BOARD_CF_SECTOR_SIZE) {
        // Sektor 1: Bytes 0-255
        fread(b->cf_sector, 1, img_sec, b->cf_file);
        // Sektor 2: Bytes 256-511
        fread(b->cf_sector + img_sec, 1, Q9_BOARD_CF_SECTOR_SIZE - img_sec, b->cf_file);
    }
}

// WRITE: Schrieb nur 1 Sektor (0-255)!
static void cb030_cf_store_sector(q9_board_t *b) {
...
static void cb030_cf_store_sector(q9_board_t *b) {
    if (cb030_cf_ensure_open(b)) {
        uint32_t img_sec = b->cf_image_sector_size ? b->cf_image_sector_size : Q9_BOARD_CF_SECTOR_SIZE;
...
- `src/kernel/q9board.c`: Fix in `cb030_cf_store_sector()`
- `src/kernel/q9board.h`: Feld `cf_image_sector_size` dokumentiert
