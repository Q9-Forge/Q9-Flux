...
| 5.2b | 68681-DUART (seriell) | 🟢 | Claudia | Minimalansatz: JEDER Registerzugriff muss sauber angenommen werden (Init-Sequenz des Boot-ROMs), aber nur SRA (Status) + THRA/RHRA (Zeichenpuffer) müssen wirklich funktionieren — THRA-Schreibzugriff gibt das Zeichen an die Q9-Konsole aus. Volles Register-Set: `docs/CB030.md`. Andockung an `cb030.c` (5.2a) — I/O-Bereich `0xFFFF_F000`–`0xFFFF_FFFF`, der dort noch kommentarlos verworfen wird |
| 5.2c | Compact-Flash-Interface | 🟢 | Claudia | Backing Store: große Datei (Vorschlag 4 GB), Zugriff über Host-Datei-I/O — Muster von `q9disk.img` (3.1) wiederverwendbar. ATA-PIO-Kommandoprotokoll: mindestens READ SECTOR(S) (`0x20`) + WRITE SECTOR(S) (`0x30`) ins `CFstat`-Kommandoregister, Standard-ATA-Statusbits (BSY=`0x80`, DRQ=`0x08`, RDY=`0x40`, ERR=`0x01`) — Industriestandard, keine CB030-Spezifika zu erwarten, bei Bedarf später anpassen. Details: `docs/CB030.md` |
| 5.2d | Timer/IRQ3 | 🟢 | Claudia | `TI_IRQ_ON`/`TI_IRQ_OFF` sind reine Adress-Trigger (kein Datenwert), 100 Hz. Kooperative Umsetzung (kein echter Host-Timerinterrupt — würde Nebenläufigkeit in Musashis nicht-threadsicheren Zustand einführen, widerspricht E8): Host-Uhrzeit zählen (`q9_hal_ticks_ms()`), bei ≥10ms seit letztem Auslösen `m68k_set_irq(3)` aufrufen — Musashi erledigt Stack/Vektorsprung selbst. Details + Pseudocode: `docs/CB030.md` |
...
**Letzte Aktualisierung**: 2026-07-04 abends — **5.2d (Timer/IRQ3) freigegeben,
