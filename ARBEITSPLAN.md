...
| 5.5 | CF-Image bespielen — OS-9 mit echtem Dateisystem (und perspektivisch CF-Boot) | 🟢 | Andreas + Claudia | Andreas hat fertige CF-Images (evtl. sogar mit C-Compiler!) und baut morgen (2026-07-06) eines ein. Unser `cb030_cf.img` ist ein rohes LBA-Sektor-Image — vorhandenes Image einfach dorthin kopieren (oder Pfad per Option einstellbar machen, dann bleibt das Original unangetastet). **Bekannte Lücke, die dabei vermutlich zuschlägt**: Die CF-Emulation (`cb030_cf_read/write`) bedient bisher genau EINEN Sektor pro READ/WRITE SECTOR(S)-Kommando und ignoriert `cf_sectcnt` — wenn der cfide-Treiber Multi-Sektor-Transfers anfordert, muss der Zähler richtig durchgezählt werden (Sektor fertig → nächsten laden/schreiben, DRQ halten, bis Count erschöpft). Stelle ist vorbereitet, schneller Fix sobald es auftritt. Fernziel: `os9gen` aufs CF → Boot von CF statt ROM |
