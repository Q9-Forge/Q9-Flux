//════════════════════════════════════════════════════════════════════════════════════════════════
// File:   q9_launcher.c                                                                  Ver. 1.00
// Owner:  Cld
// Desc.:  Implementierung, siehe q9_launcher.h -- REIN VERSCHOBEN aus demo/integration_demo.c
//         (Ver. 1.00-3.90 dort traegt die volle Entstehungsgeschichte jedes einzelnen Details:
//         Farbpalette, Feld-Definitionen, Datei-Dialog-Integration, Resize-Overlay, CF-Image-
//         Slots, echte devdesc-Hardware-Eintraege). Neu in dieser Runde: build_current_cfg()
//         (aus save_q9_config() ausgelagert, jetzt von save_q9_config() UND der neuen Taste 'B'
//         genutzt) und die Taste 'B' selbst (Booten -- beendet die Schleife, liefert die
//         Config an den Aufrufer statt sie wie 'S' auf die Platte zu schreiben).
//
// Edition History
//─────────┬──────┬────────────────────────────────────────────────────────────────────────┬──────
...
//         │      │ demo/integration_demo.c, neue Taste B (Booten) + build_current_cfg()      │
//═════════╧══════╧════════════════════════════════════════════════════════════════════════╧══════
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
/* Erster fester Eintrag -- KEIN Hardware-Ding wie die Eintraege darunter, sondern die eigentliche
   ...
   Q9_KEY_ENTER-Behandlung in q9_launcher_run()).
...
/* MAX_ITEM_COUNT: volle Array-Kapazitaet (Compile-Zeit-Konstante fuer g_list_items[]/g_expanded[]s
   ...
   CF-Image-Slots. g_list_items[] ist seit der Vierunddreissigsten Runde NICHT MEHR const
   (init_items() befuellt Name/Felder/field_count zur Laufzeit aus der devdesc-Registry, s.o.) --
   anders als z.B. g_cfg_fields/g_cfimg_fields aendert sich hier auch .name selbst, nicht nur
   .fields[].value. g_base_item_count (Laufzeit-Variable, ersetzt das fruehere Compile-Zeit-Macro
   BASE_ITEM_COUNT): Emulator-Konfiguration + tatsaechliche Anzahl devdesc-Hardware-Eintraege
   (haengt von q9_devdesc_count() ab, s. init_items()), OHNE die CF-Image-Slots. g_item_count
   (Laufzeit, ebenfalls in init_items() gesetzt) ist die tatsaechlich AKTIVE Anzahl -- startet bei
...
/* 0 = zugeklappt (Default), 1 = aufgeklappt -- Pfeil rechts auf der Hauptliste klappt den
   ...
   auch wenn q9_launcher_run() sie nie erreicht, solange g_item_count kleiner ist). */
...
/* Baut g_list_items[]/g_base_item_count/g_item_count einmalig zur Laufzeit auf (s. Kommentar
   oben) -- MUSS vor dem ersten Zugriff (q9_launcher_run()s q9_listview_init()) aufgerufen werden.
   Index 0 bleibt "Emulator-Konfiguration" (unveraendert), Indizes [1 .. g_base_item_count) kommen
   aus der devdesc-Registry (jeder Typ ausser "cf"), direkt danach die vier CF-Image-Slots. */
...
/* Baut den vollen Hauptbildschirm-Inhalt in sb auf (KEIN stdout-Schreiben) -- ausgelagert aus
   ...
   macht das Ergebnis des Datei-Auswahl-Dialogs (task #22) sichtbar, ohne die Statuszeile selbst
   (feste Feldbreiten, s.o.) umbauen zu muessen. */
                                ...
                                const char *hint)
{
    ...
    q9_screenbuf_puts(sb, 0, 3, "Q9-Flux Configurator",
                       PAL_HEADER_FG_R, PAL_HEADER_FG_G, PAL_HEADER_FG_B);
    ...
    q9_screenbuf_puts(sb, rows - 2, 3,
                       (hint && hint[0]) ? hint
                                         : "Pfeiltasten: navigieren   Rechts: oeffnen+bearbeiten   "
                                           ...
                                           "S: speichern   B: Booten   N: CF-Image anlegen   "
                                           "D: CF-Image loeschen   Strg-C: beenden",
                       PAL_FRAME_R, PAL_FRAME_G, PAL_FRAME_B);
...
/* Sechsundzwanzigste Runde ("Speichern-Funktion"): die zuletzt erfolgreich GELADENE Konfiguration
   ...
   anzeigt/bearbeitet) -- build_current_cfg()/save_q9_config() starten davon (statt bei leeren
   Defaults) und ueberschreiben nur die Felder, die der Editor tatsaechlich zeigt. Ohne das wuerden
   beim Speichern/Booten einer geladenen Datei ihre [cfN]-Abschnitte stillschweigend verschwinden.
   g_cfg_loaded==0 bedeutet "noch nie erfolgreich geladen (oder letzter Ladeversuch ist
   fehlgeschlagen)" -- startet dann bei q9_board_cfg_default() (frische Config, kein [cfN]), deckt
   den "neue Config von Grund auf" Fall ab. */
...
/* Wandeln q9_cfg_cf_t.format/bus/unit (Enum-Werte) in genau die Kurztexte um, die auch die
   ...
   parse_cf_format()/_bus()/_unit() bei build_current_cfg(). BEWUSST hier dupliziert statt
...
/* Taste 'N': naechsten freien CF-Image-Slot aktivieren (g_item_count++, s. g_base_item_count-
   Kommentar), Defaults setzen, Auswahl+Scroll draufsetzen. build_current_cfg() braucht dafuer
   KEINE Anpassung -- cf_visible = g_item_count - g_base_item_count wird dort schon bei jedem
   Speichern/Booten neu berechnet. Voll (alle Q9_LISTVIEW_CFIMG_SLOTS aktiv): No-op, msg erklaert
   warum. */
...
/* Taste 'D': den AUSGEWAEHLTEN CF-Image-Slot entfernen -- nur wirksam, wenn die Auswahl
   ...
   versehentliches Loeschen eines Hardware-Eintrags durch denselben Tastendruck). Slots bleiben
   IMMER lueckenlos am Anfang (Position i = CF-Image #i, s. Kopfkommentar bei g_cfimg_fields) --
   nachfolgende Slot-INHALTE ruecken deshalb eine Position nach vorn (memcpy, Label/Kind sind je
   Zeile ohnehin identisch), statt nur die Sichtbarkeitsgrenze zu verschieben. Alle CF-Image-
   Eintraege klappen dabei zu (g_expanded zurueckgesetzt) -- verschobene Slot-Inhalte hinter einem
   weiterhin aufgeklappten Eintrag waeren sonst verwirrend, einfacher Neustart. */
...
/* Andreas' Wunsch (2026-08-18, fuenfundzwanzigste Runde): "echtes Laden/Auswerten der .q9-Datei"
   ...
   q9_board_cfg_load()); g_cfg_loaded bleibt/wird 0 (kein Speichern/Booten auf Basis einer kaputten
   Ladung, s. build_current_cfg()) UND vorhandene CF-Image-Eintraege verschwinden wieder (cf_count
...
/* Uebernimmt field_value in out -- die Platzhalter "<leer>"/"<Fehler>" (reine ANZEIGE-Werte, s.
   ...
   andere woertlich uebernommen. Kleine Hilfsfunktion fuer build_current_cfg() unten. */
...
/* q9_cfg_cf_t.format/bus/unit sind Enum-Werte -- kehrt cf_format_str()/_bus_str()/_unit_str() um
   (fuers Uebernehmen der CF-Image-Felder Typ:/Bus:/Unit:, s. build_current_cfg()). fallback greift
   bei unbekanntem Text (z.B. Tippfehler) -- BEHAELT den vorherigen (aus g_loaded_cfg
   uebernommenen) Wert bei, statt auf einen willkuerlichen Default zu springen; ein wirklich
   falscher Wert faellt spaetestens beim naechsten Laden auf (s. Kommentar bei cf_format_str()
   oben). */
...
/* Fuenfunddreissigste Runde: aus save_q9_config() ausgelagert (dort Ver. 3.50-3.90 fuer die
   volle Entstehungsgeschichte) -- baut die AKTUELLEN Feldwerte (Name:/ROM:/Netz:/CPU:, alle
   sichtbaren CF-Image-Slots) in *cfg, egal ob spaeter gespeichert (save_q9_config()) oder direkt
   gebootet wird (Taste B, s. q9_launcher_run()). Startet bei g_loaded_cfg (falls vorhanden, s.
   Kommentar dort) statt bei leeren Defaults, damit unbekannte [cfN]-Felder (Basis/Slot/
   Descriptor -- der Editor zeigt/bearbeitet die noch nicht, s. "Noch offen" in
   Q9FLUX_EDITOR_de.md) NICHT stillschweigend verschwinden. */
static void build_current_cfg(q9_board_cfg_t *cfg)
{
    int i, cf_visible;
    ...
    /* cf_visible == cfg.cf_count per Konstruktion (sync_cfimg_items() haelt g_item_count IMMER
       ...
       nicht stillschweigend vorausgesetzt (robust gegen kuenftige Aenderungen). */
...
/* Andreas' Wunsch (2026-08-18, sechsundzwanzigste Runde): "Speichern-Funktion" -- schreibt die
   AKTUELLEN Feldwerte (build_current_cfg(), s.o.) in die Datei zurueck, die im Datei:-Feld steht.
...
/* Oeffnet den modalen Datei-Auswahl-Dialog (q9_filedialog.h/.c, task #20) zentriert ueber dem
   ...
   rows/cols sind Zeiger (nicht nur Eingabe) -- ein Terminal-Resize WAEHREND der Dialog offen ist
   wird behandelt (Andreas' Feedback, elfte Runde: "Fenster Groesse aendern waehrend ein Dialog
   auf ist funktioniert nicht richtig"). Overlay-Settle-Muster wie q9_launcher_run() (s.
   ...
   dabei automatisch neu) passiert erst EINMAL, nach RESIZE_SETTLE_MS Stille. BEKANNTE
   VEREINFACHUNG: das Neu-Initialisieren setzt Fokus/Auswahl/Filter auf ihre Startwerte zurueck
   (kein Nachziehen des BISHERIGEN Zustands) -- Resize mitten in der Dateiauswahl ist ein Randfall.
   Die Zeiger werden aktualisiert, damit der Aufrufer nach Rueckkehr die dann aktuelle Terminal-
   Groesse kennt. Strg-C/EOF waehrend des Dialogs wird NICHT ignoriert (sonst liesse sich das
   Programm aus dem Dialog heraus nicht mehr beenden) -- signalisiert per Rueckgabe 0 an den
   Aufrufer, der dann seinerseits sauber beendet.
                            ...
                            char *out_name, unsigned out_name_size)
{
    /* Andreas' Wunsch (2026-08-18, zwanzigste Runde): "als Verzeichnis sollte ~/.q9-flux gesetzt
       sein und dort sollen alle *.q9 Dateien angezeigt werden". ".q9" (OHNE fuehrenden Stern) als
       Standardfilter -- q9_filelist.c's ext_matches() erwartet eine reine Endung (mit oder ohne
       fuehrenden Punkt, s. dort), KEIN Glob-Muster wie "*.q9"; "*.q9" haette NIE getroffen (beim
       ersten Testlauf per pyte tatsaechlich aufgefallen: leere Liste trotz vorhandener
       .q9-Dateien). "*.*" bleibt als Ausweichoption im Dropdown (z.B. um zu sehen, ob ueberhaupt
       etwas im Verzeichnis liegt). Verzeichnis wird bei Bedarf angelegt (mkdir, Fehler bewusst
       ignoriert -- existiert es schon, ist das kein Problem; schlaegt es aus anderem Grund fehl,
       zeigt der Dialog einfach eine leere Liste, s. q9_filedialog.c rescan()). */
    ...
    int done = 0;
    int want_quit = 0;
    int showing_overlay = 0;                                 /* wie q9_launcher_run(): billiges
                                                                 Groessen-Overlay waehrend des
                                                                 Ziehens statt teurem Dialog-Redraw
                                                                 bei jedem Zwischenschritt, s.o. */
...

        {
            ...
            if (showing_overlay && k.kind == Q9_KEY_NONE) {
                /* RESIZE_SETTLE_MS ohne weiteres Resize-Ereignis abgelaufen -- JETZT (und nur
                   ...
                   Filtern/Verzeichnis/Palette neu aufsetzen. Bleibt das Fenster zu klein, einfach
                   im Overlay bleiben (naechste Runde zeichnet es idempotent neu). */
                if (!too_small) {
                    compute_dialog_geometry(*rows, *cols, &dlg_row, &dlg_col, &dlg_rows, &dlg_cols);
                    if (q9_filedialog_init(&dlg, dlg_row, dlg_col, dlg_rows, dlg_cols,
                                            "Konfigurationsauswahl", dir, filters, 2, &pal) != 0) {
    ...
    if (done == 1) {
        ...
        /* out_name ist optional (NULL = Aufrufer interessiert sich nur fuer die Hinweis-Nachricht,
           s. der 'o'/'O'-Aufruf in q9_launcher_run()) -- der Button-Feld-Aufruf (Emulator-
           Konfiguration) will dagegen den rohen Dateinamen, um ihn ins Namensfeld zu uebernehmen. */
...
/* Andreas' Wunsch (2026-08-18, dreiundzwanzigste Runde): "Numerische Eingabe Dezimal/Hex opt. mit
   ...
   Verlassen des Feldes auf die naechstliegende Grenze gezogen. */
...
/* Ruft clamp_field_range() fuer das gerade fokussierte Feld auf, WENN es das "Slot:"-Beispiel ist
   (per Label erkannt). Wird beim Verlassen des Feldes aufgerufen (Pfeil links UND Esc, s.
   q9_launcher_run()).
   ...
   wieder, ohne q9_launcher_run()s Aufrufstellen anfassen zu muessen. */
...
int q9_launcher_run(q9_board_cfg_t *out_cfg)
{
    ...
    q9_listview_t lv;
    int running = 1;
    int want_boot = 0;                                      /* 1 = Taste B gedrueckt, out_cfg     */
                                                              /* gilt beim Rueckgabewert            */
    ...
    /* Muss vor dem ersten Zugriff (gleich beim q9_listview_init() unten) gebaut sein. */
    init_items();

    if (q9_input_init() != 0) {
        fprintf(stderr, "q9_launcher_run: kein echtes Terminal (TTY) -- Abbruch.\n");
        return 0;
    }
...

        {
            ...
            switch (k.kind) {
                ...
                case Q9_KEY_ENTER:
                    ...
                    if (lv.field_focus < 0) {
                        /* Kurzform auf Item-Ebene -- klappt den AUSGEWAEHLTEN Eintrag auf/zu, OHNE
                           in die Felder zu springen (das macht gezielt Pfeil rechts). */
                        ...
                        if (r == 1 && lv.field_focus > 0) {
                            ...
                            /* Nicht nur den Namen uebernehmen, sondern die Datei auch WIRKLICH
                               laden (ueberschreibt last_dialog_msg mit dem genaueren Lade-
                               Ergebnis statt der reinen Auswahl-Bestaetigung). */
                ...
                case Q9_KEY_CHAR:
                    /* Waehrend ein Feld fokussiert ist: das Zeichen geht DIREKT in den Feldwert
                       ...
                       SONDERFALL Leertaste bei einem BOOLEAN-Feld -- schaltet ja/nein um statt (wie
                       bei field_putc() ohnehin wirkungslos) einfach zu verpuffen. */
                    if (!showing_overlay && lv.field_focus >= 0
                        && k.ch == ' '
                    ...
                    } else if (!showing_overlay && (k.ch == 'b' || k.ch == 'B')) {
                        /* Fuenfunddreissigste Runde (Andreas: "ich wollte eigentlich das man in
                           den Configurator kommt wenn man den emulator ohne parameter aufruft") --
                           Booten: baut die AKTUELLEN Feldwerte (wie Speichern, aber OHNE zu
                           speichern) in out_cfg und beendet die Schleife. Der Aufrufer (q9.exe
                           selbst, s. hal_posix.c) bootet damit im selben Prozess direkt weiter --
                           kein Speichern-Zwang, man kann eine Config "probeweise" starten. */
                        build_current_cfg(out_cfg);
                        want_boot = 1;
                        running = 0;
